...
function startFallback(){stopFallback();var run=fallbackRun;setStatus('LIVE TO PC · SECURE FALLBACK',true);send({type:'fallback-started'});var context=transport.getContext('2d',{alpha:false});function next(delay){if(run===fallbackRun)fallbackTimer=setTimeout(pump,delay);}function pump(){if(run!==fallbackRun)return;if(!context||!socket||socket.readyState!==1||!stream||preview.readyState<2||socket.bufferedAmount>700000){next(70);return;}var width=960;var height=540;transport.width=width;transport.height=height;var sw=preview.videoWidth||width;var sh=preview.videoHeight||height;var scale=Math.min(width/sw,height/sh);var dw=Math.round(sw*scale);var dh=Math.round(sh*scale);context.fillStyle='#000';context.fillRect(0,0,width,height);context.drawImage(preview,(width-dw)/2,(height-dh)/2,dw,dh);transport.toBlob(function(blob){if(!blob){next(70);return;}blob.arrayBuffer().then(function(buffer){if(run===fallbackRun&&socket&&socket.readyState===1&&socket.bufferedAmount<700000)socket.send(buffer);}).finally(function(){next(55);});},'image/jpeg',0.76);}pump();}
  ...
  socketServer.on('connection', (webSocket) => {
    ...
    webSocket.on('message', (data, binary) => {
      ...
      try {
        ...
        // Only means ICE found a path. The renderer promotes this to
        // 'streaming' once it has actually decoded a frame.
        if (message?.type === 'media-connected') options.onState('connected', 'Phone connected. Checking that video is really arriving…')
